...
`@part1`
Often times, as we move large data files around, connections can timeout.  To make sure that our download resumes and progress is not lost, `curl` has two particularly useful option flags:
...
The order of the flags specified will not alter the outcome.
